...
/* ---------------------------------------------------------------------------
 * Extended options for persona-aware calls
 * ------------------------------------------------------------------------- */
export interface GenerateOptions {
  /** Debate mode hint forwarded to serverless proxy */
  mode?: 'Debate' | 'Lecture';
  /** Optional retrieved context snippets for RAG grounding */
 ...
 * @param options - Optional debate/lecture mode and RAG context
  ...
  try {
    const res = await fetch('/api/openai/chat', {
      ...
      body: JSON.stringify({
        ...
        characterPersona,
        messages,
        // Only include extra fields if provided to avoid server-side 400s
        ...(options?.mode ? { mode: options.mode } : {}),
        ...(options?.ragContext ? { ragContext: options.ragContext } : {}),
      }),
    });
