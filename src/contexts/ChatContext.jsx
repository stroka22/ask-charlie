...
export const ChatProvider = ({ children }) => {
  ...
  /* Debate/Lecture mode (default Debate) */
  const [debateMode, setDebateMode] = useState('Debate'); // 'Debate' | 'Lecture'
  /* ------------------------------------------------------------------
    ...
    try {
      ...
      try {
        if (import.meta.env?.VITE_ENABLE_RAG === 'true') {
          ragContext = await ragRetrieve(userMessage.content, 5);
        }
      } catch (e) {
        console.warn('[ChatContext] RAG retrieval failed:', e);
      }
      ...
      /* Ensure required challenge suffix is present */
      const SUFFIX =
        'What’s the next argument you’d like to test against my worldview?';
      let finalReply = reply || '';
      if (!finalReply.trim().endsWith(SUFFIX)) {
