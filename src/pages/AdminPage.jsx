...
const AdminPage = () => {
    ...
    const handleDeleteSelected = async () => {
        ...
        if (!window.confirm(`Are you sure you want to delete ${selectedCharacters.length} assistant(s)? This action cannot be undone.`)) {
            return;
        }
        ...
        try {
            ...
            setSuccessMessage(`Successfully deleted ${selectedCharacters.length} assistant(s).`);
            setSelectedCharacters([]);
            fetchCharacters();
        } catch (err) {
            console.error('Bulk character deletion error:', err);
            setError(err instanceof Error ? err.message : 'Failed to delete selected assistants.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const exportCharactersToCSV = () => {
        ...
        for (const character of characters) {
            const row = columns.map(column => {
                switch(column) {
                    case 'name': return escapeCSV(character.name);
                    case 'short_description': return escapeCSV(character.short_biography || '');
                    case 'description': return escapeCSV(character.description || '');
                    case 'persona_prompt': return escapeCSV(character.persona_prompt || '');
                    case 'opening_line': return escapeCSV(character.opening_line || '');
                    case 'avatar_url': return escapeCSV(character.avatar_url || '');
                    case 'feature_image_url': return escapeCSV(character.feature_image_url || '');
                    case 'is_visible': return escapeCSV(character.is_visible ?? true);
                    case 'reference_tag': return escapeCSV(character.bible_book || '');
                    case 'timeline_period': return escapeCSV(character.timeline_period || '');
                    case 'historical_context': return escapeCSV(character.historical_context || '');
                    case 'geographic_location': return escapeCSV(character.geographic_location || '');
                    case 'reference_tags': return escapeCSV(character.key_scripture_references || '');
                    case 'significance_notes': return escapeCSV(character.theological_significance || '');
                    case 'relationships_json': return escapeCSV(character.relationships ? JSON.stringify(character.relationships) : '{}');
                    case 'study_questions': return escapeCSV(character.study_questions || '');
                    case 'context_notes': return escapeCSV(character.scriptural_context || '');
                    default: return escapeCSV(character[column]);
                }
            });
            csv += row.join(',') + '\n';
        }
        ...
        link.setAttribute('download', `assistants-export-${date}.csv`);
    ...
    const handleCSVUpload = async (event) => {
        ...
        try {
            ...
            const charactersToCreate = parsedData.map(row => ({
                ...
                bible_book: row.reference_tag || row.bible_book || '',
                opening_line: row.opening_line || row.opening_sentence || '',
                persona_prompt: row.persona_prompt || '',
                scriptural_context: row.context_notes || row.scriptural_context || '',
                ...
                key_scripture_references: row.reference_tags || row.key_scripture_references || '',
                theological_significance: row.significance_notes || row.theological_significance || '',
                relationships: tryParseJson(row.relationships_json || row.relationships) || {},
            ...
            if (charactersToCreate.length === 0) {
                throw new Error('No valid assistants found in CSV. Ensure headers and data are correct.');
            }
    ...
    const handleToggleVisibility = async (character) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Failed to toggle assistant visibility.');
        }
        finally {
            setIsLoading(false);
        }
    ...
    return (_jsxs("div", { className: "container mx-auto px-4 pt-24 pb-8", children: [_jsx("h1", { className: "text-3xl font-bold text-gray-900 mb-6", children: "Admin Panel - Assistant Management" }), _jsx("p", { className: "text-gray-700 mb-4", children: "Welcome! Manage your assistants and related data here." }), 
        ...
        <div className="mb-8 border-b border-gray-200">
          <nav className="-mb-px flex space-x-8" aria-label="Admin Tabs">
            <button onClick={() => setActiveTab('characters')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'characters' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-900 hover:text-gray-900 hover:border-gray-300'}`}>Assistants</button>
            ...
            <button onClick={() => setActiveTab('featured')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'featured' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-900 hover:text-gray-900 hover:border-gray-300'}`}>Featured Assistant</button>
            ...
            <button onClick={() => setActiveTab('studies')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'studies' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-900 hover:text-gray-900 hover:border-gray-300'}`}>Content Studies</button>
