    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Panel */}
          <motion.div
            className="fixed right-0 top-0 h-full w-full max-w-md bg-gradient-to-b from-[#0b1020] via-[#0f172a] to-[#111827] shadow-2xl z-50 overflow-y-auto p-6"
          ...
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-4 right-4 text-cyan-300 hover:text-cyan-400 transition-colors"
            ...
            <div className="text-center mb-8 pt-4">
              <img
                ...
                alt={character.name}
                className="h-24 w-24 rounded-full object-cover mx-auto mb-4 border-4 border-cyan-400 shadow-lg"
                onError={(e) => {
              ...
              <p className="text-white text-lg">{character.description}</p>
            </div>
            ...
            <div className="space-y-8">
                ...
                {character.key_scripture_references ? (
                  <div className="text-white text-sm space-y-1">
                    {character.key_scripture_references
                      ...
                      .map((ref, index) => (
                        <div key={index}>
                          <ScriptureReference reference={ref.trim()} className="text-cyan-300 hover:underline" />
                        </div>
                      ))}
                  </div>
                ) : (
                  <p className="text-white text-sm">No key scripture references available.</p>
                )}
              </section>
              ...
              <section className="bg-white/10 backdrop-blur-sm rounded-lg p-4 shadow-md border border-white/20">
                ...
                Object.keys(character.relationships).length > 0 ? (
                  ...
                  <p className="text-white text-sm">No relationship data available.</p>
                )}
              </section>
              ...
              <section className="bg-white/10 backdrop-blur-sm rounded-lg p-4 shadow-md border border-white/20">
                ...
                {character.study_questions ? (
                  ...
                  <p className="text-white text-sm">No study questions available.</p>
                )}
              </section>
            </div>
          </motion.div>
