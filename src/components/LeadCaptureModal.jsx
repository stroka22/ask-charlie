    ...
    <div className="fixed inset-0 z-[60]">
      ...
      <div className="relative w-full h-full flex items-center justify-center p-4">
        <div role="dialog" aria-modal="true" aria-labelledby="lead-modal-title" className="w-full max-w-xl rounded-2xl overflow-hidden shadow-2xl">
          <div className="bg-gradient-to-r from-[#0b1020] to-[#0f172a] text-white px-6 py-4">
            <div className="flex items-start">
              <div className="flex-1">
                <h2 id="lead-modal-title" className="text-xl font-semibold">Get product updates and tips</h2>
          ...
          {success ? (
            <div className="bg-white px-6 py-8 text-gray-900 text-center">
              <p className="text-lg font-semibold mb-4">{success}</p>
              <button
                onClick={closeAndRemember}
                className="mt-2 px-5 py-2 rounded-full bg-primary-500 text-white font-semibold shadow hover:bg-primary-600"
              >
                Close
              </button>
            </div>
          ) : (
          <form onSubmit={onSubmit} className="bg-white px-6 py-5 text-gray-900">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input value={name} onChange={(e)=>setName(e.target.value)} className="px-3 py-2 rounded-full bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-400" placeholder="Name" />
              <input value={email} onChange={(e)=>setEmail(e.target.value)} className="px-3 py-2 rounded-full bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-400" placeholder="Email*" />
              <input value={phone} onChange={(e)=>setPhone(e.target.value)} className="px-3 py-2 rounded-full bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-400" placeholder="Phone" />
              <select value={role} onChange={(e)=>setRole(e.target.value)} className="px-3 py-2 rounded-full bg-white border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary-400">
            ...
            <div className="mt-5 flex justify-end">
              <button type="button" onClick={closeAndRemember} className="mr-3 px-4 py-2 rounded-full border border-gray-300 text-gray-900 hover:bg-gray-100">Not now</button>
              <button disabled={submitting} className="px-5 py-2 rounded-full bg-primary-500 text-white font-semibold shadow hover:bg-primary-600 disabled:bg-gray-400">{submitting ? 'Submitting...' : 'Keep me updated'}</button>
            </div>
          </form>
          )}
        </div>
