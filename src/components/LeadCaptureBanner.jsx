...
export default function LeadCaptureBanner() {
  ...
  return (
    <div
      className="fixed left-0 right-0 z-40 bg-gradient-to-r from-[#0b1020] to-[#0f172a] text-white shadow-lg"
      style={{ top: offset }}
    >
      <div className="container mx-auto px-4 py-3">
        {success ? (
          <div className="rounded-xl border border-white/10 backdrop-blur-sm px-4 py-4 flex items-center justify-between">
            <span className="text-green-200 text-sm md:text-base">{success}</span>
            <button
              type="button"
              onClick={dismiss}
              className="ml-4 px-4 py-1 rounded-full bg-primary-500 text-white font-semibold shadow hover:bg-primary-600 text-sm"
            >
              Close
            </button>
          </div>
        ) : (
        <form onSubmit={onSubmit} className="rounded-xl border border-white/10 backdrop-blur-sm px-4 py-3 grid grid-cols-1 md:grid-cols-8 gap-2 items-center">
          <div className="md:col-span-2">
            <div className="font-semibold">Get product updates and tips</div>
          ...
          <input
            ...
            className="md:col-span-1 px-3 py-2 rounded-full bg-white text-gray-900 placeholder-gray-500 border-transparent focus:outline-none focus:ring-2 focus:ring-primary-400"
            placeholder="Name"
          />
          <input
            value={email}
            onChange={(e)=>setEmail(e.target.value)}
            className="md:col-span-2 px-3 py-2 rounded-full bg-white text-gray-900 placeholder-gray-500 border-transparent focus:outline-none focus:ring-2 focus:ring-primary-400"
            placeholder="Email*"
          />
          <input
            value={phone}
          ...
          <select
            ...
            className="md:col-span-1 px-3 py-2 rounded-full bg-white text-gray-900 border-transparent focus:outline-none focus:ring-2 focus:ring-primary-400"
          >
          ...
          <button
            disabled={submitting}
            className="md:col-span-1 flex items-center justify-center gap-1 px-5 py-2 rounded-full bg-primary-500 text-white font-semibold shadow hover:bg-primary-600 disabled:bg-gray-400"
          >
