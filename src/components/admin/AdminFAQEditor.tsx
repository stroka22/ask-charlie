...
const AdminFAQEditor: React.FC = () => {
  ...
  const [faqs, setFaqs] = useState<Array<Faq & { isPublished?: boolean; createdAt?: string; updatedAt?: string }>>([]);
  ...
  /* ------------------------------------------------------------------
   * Admin detection (simplified - always admin)
   * ------------------------------------------------------------------ */
  const isAdminUser = true; // Simplified: always admin
  ...
  const handleEdit = (faq: typeof faqs[0]) => {
    ...
    // Use null fallback so TypeScript knows state may be null
  ...
  const handleToggleVisibility = (id: string) => {
    ...
    (async () => {
      if (isAdminUser) {
        try {
          // Cast because we're only sending a partial FAQ payload
          await upsertFaqRow({ id, is_published: nextPublished } as any);
        } catch (err) {
